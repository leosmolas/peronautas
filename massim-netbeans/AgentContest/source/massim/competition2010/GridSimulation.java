...
public class GridSimulation extends AbstractGridSimulation {
	...
	private void cowSimulation() {
		...
		for (int i = 0; i < state.cows_manager.size(); i++) {
			if (this.getSteps() % state.cowSpeed == state.cows_manager.get(i).cowTurn) {
				...
				currCow.computerCowMove2(state);
	...
	// FIXME IndexOutOfBoundsException possible!!!
			...
			for (int z = 0; z < agents.length; z++) {
				...
				if (a_state.posx == x && a_state.posy == y) {
					...
					if (l_r.contains(dir) && l_r.contains(switch_dir)
							|| u_d.contains(dir) && u_d.contains(switch_dir)) {
						// find a new position around of agent, where no object
						// exists
						pointXY = this.findFreePos(x, y, state);
						shiftAgent(a_state, x, y, pointXY[0], pointXY[1]);
						return true;
					}
					/*
					 * if(l_r.contains(dir)&& l_r.contains(switch_dir)){
					 ...
					 * !state.board[a+1][b].noObject() ) dir = "up";
					 * if(dir.equalsIgnoreCase("right") &&
					 * !state.board[a-1][b].noObject() ) dir = "down";
					 * 
					 * } if(u_d.contains(dir)&& u_d.contains(switch_dir)){
					 * 
					 * if(dir.equalsIgnoreCase("up") &&
					 * !state.board[a][b+1].noObject() ) dir = "left";
					 * if(dir.equalsIgnoreCase("down") &&
					 * !state.board[a][b-1].noObject() ) dir = "right";
					 * 
					 * 
					 * }
					 */
					// charge position of agent
					pointXY = getNewPosition(a, b, dir);
	...
	private void shiftAgent(GridSimulationAgentState a_state, int fromx,
		...
		a_state.direction = this.getDirection(tox, toy, fromx, fromy);
	...
	public void postSimulationStep() {
		...
		// count the cows in the corrals
		state.cowsInCorral[0] = 0;
		state.cowsInCorral[1] = 0;
		...
		while (iter.hasNext()) {
			...
			if (state.board[cow.posx][cow.posy].isStable1()) {
				state.cowsInCorral[0] += 1;
			} else if (state.board[cow.posx][cow.posy].isStable2()) {
				state.cowsInCorral[1] += 1;
			}
		}

		// calculate average score
		state.cowsSum[0] += state.cowsInCorral[0];
		state.averageScore[0] = ((double) state.cowsSum[0])
				/ ((double) state.currentStep);
		state.cowsSum[1] += state.cowsInCorral[1];
		state.averageScore[1] = ((double) state.cowsSum[1])
				/ ((double) state.currentStep);
	...
	public String finalizeSimpleSimulation() {
		...
		GridSimulationWorldState state = (GridSimulationWorldState) this
				.getSimpleSimulationState();
		String winner = "";

		if (state.averageScore[0] < state.averageScore[1]) {
			winner = state.teamName[1];
		}

		else if (state.averageScore[0] > state.averageScore[1]) {
